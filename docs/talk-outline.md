...
1944字。レビュー2回目で「Wikipediaを直接の根拠として強く出すのは危うい」「Kotlinの抽象コストは"実質ゼロ"は言い過ぎ」と指摘され、Wikipedia由来の記述を「定石として紹介されている」に弱め、"実質ゼロ"を"支配的ではなかった"に修正。3回目のレビューで(3)冒頭にも同じ強い表現が残っていた点を修正。4回目のレビュー（claude web）で、(4)が15分の配分に対して薄いこと、(1)に問いが無いまま(2)へ繋がること、(2)にΘ(V)のメモリモデルが無いままΘ(V)の話が(4)末尾で唐突に出ることを指摘され、(1)末尾に問いを追加、(2)にΘ(V)/Θ(A+a_max)のメモリモデルを追加、(4)に測定設計（Prometheus/EXPLAIN ANALYZE）とJIT仮説検証・棄却の経緯を追加。5回目のレビューで「集約自体が原因と分かった」という断定が強すぎると指摘され、「条件付き集約の重さが主因と見られた」に弱めた。
...
・SQL PivotはWideで条件付き集約が重く、JITコストが原因という仮説も検証したが、無効化しても改善せず、条件付き集約の重さが主因と見られた
